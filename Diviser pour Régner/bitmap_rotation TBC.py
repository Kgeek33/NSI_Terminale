...
# si 0<=x<largeur et 0<=y<hauteur
# la couleur du pixel (x,y) est donnée par px[x,y]
px = img.load()
# on peut modifier la couleur d'un pixel (x,y) avec
# l'affectation px[x,y]=c où c est une couleur
PIL.Image.show(img)
...
def rotation(px, x, y, t):
    ...
    # cas de base : un pixel tout seul : il tourne sur lui-même
    # et rien ne change...
    if t == 1:
        return None

    # un carré non réduit à un point ...
    # rotation des 4 quarts
    ...
    for xx in range(x, t//2):
        for yy in range(y, t//2):
            ...
            px[xx, yy] = px[xx+t//2, yy]
            px[xx+t//2, yy] = px[xx+t//2, yy+t//2]
            px[xx+t//2, yy+t//2] = px_tmp
    return None
