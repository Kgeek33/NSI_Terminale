...
def recherche_indices_classement(elt,tab):
    """prend en paramètres un entier elt et un tableau d’entiers tab
    représenté par une liste Python, et
    renvoie trois listes Python d’entiers:
    la première liste contient les indices des valeurs du tableau tab strictement inférieuresà elt ;
    ...
    for k in range(len(tab)):
        e=tab[k]
        if e==elt : L_egal.append(k)
        elif e<elt : L_inf.append(k)
        else : L_sup.append(k)
    return(L_inf,L_egal,L_sup)
...
def moyenne(nom, resultats):
    ...
    if nom in resultats: 
        notes = resultats[nom]
        if len(resultats[nom])==0: # pas de notes 
            return 0
        total_points = 0 
