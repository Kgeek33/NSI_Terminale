...
def insertion_abr(a, cle):
    ...
    if cle < racine:
        return (
            insertion_abr(sous_arbre_gauche, cle),
            racine,
            sous_arbre_droit
        )
    # insertion à droite
    if cle > racine:
        return (
            sous_arbre_gauche,
            racine,
            insertion_abr(sous_arbre_droit, cle)
        )
...
def empaqueter(liste_masses, c):
    ...
    nb_boites = 0
    # masse totale par boite : on envisage le cas le pire
    # avec autant de boites que d'objets
    boites = [0 for _ in range(n)]
    # pour chaque objet
    for masse in liste_masses:
        # recherche d'une boite d'indice i
        i = 0
        # recherche dans les boites non vides d'une boite
        # pouvant accueillir l'objet
        while i < nb_boites and boites[i] + masse > c:
            i = i + 1

        if i == nb_boites:
            # impossible de choisir une boite non vide :
            # besoin d'une boite de plus
