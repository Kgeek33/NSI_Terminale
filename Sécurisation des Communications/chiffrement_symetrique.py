...
def chiffre_cesar(msg):
    ...
    msg=msg.upper()
    for c in msg:
        msg_chiffre=msg_chiffre+d[c]   # concaténation avec le caractère crypté
    return msg_chiffre
...
def dechiffre_xor(msg_decimal,cle):
    """prend la liste msg_decimal (liste des entiers codages unicode du msg chiffré)
    et la clé en paramètres et renvoie la chaine du message déchiffré """
    "à compléter"
    return "à compléter"


print("message déchiffré : {}".format(dechiffre_xor(chiffre_xor(message,clef)[1],clef),clef))
