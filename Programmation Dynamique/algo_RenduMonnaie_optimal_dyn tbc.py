# CrÃ©Ã© le 10/03/2020 en Python 3.4
...
def nb_rendu(pieces,s,prof=None):
    """prend en paramÃ¨tre le systeme de pieces (tuple) et s, la somme Ã  rendre
    renvoie la liste des piÃ¨ces rendues"""
    ...
    for p in pieces:
        if print_appels: print(prof+str(p),end="-")
        if s-p >=0 :
            # le min permet de rÃ©initialiser le nb de pieces Ã  rendre
            nb_pieces_a_rendre = min(nb_pieces_a_rendre,1+nb_rendu(pieces,s-p,prof+" "))
        # si p>s : on ne fait rien (la combinaison examinÃ©e Ã©choue)
        if print_appels: print()
...
def nb_rendu_memo(pieces,s,dico={}):
    ...
    et dico le dictionnaire des calculs deja  effectues (programmation dynamique)
    renvoie *la taille de* la liste optimale des pieces rendues"""


    #cas de base : si la somme Ã  rendre est nulle, elle nÃ©cessite zÃ©ro piÃ¨ces
    if s == 0 : return 0
    ...
    for p in pieces:
        if s-p >=0 :
            # le min permet de rÃ©initialiser le nb de pieces Ã  rendre
            dico[s] = min(nb_pieces_a_rendre,1+nb_rendu_memo(pieces,s-p,dico))
...
#     """prend en paramÃ¨tre le systeme de pieces (tuple) et s, la somme Ã  rendre,
#     et dico_nb et dico_solt les dictionnaires des calculs dÃ©jÃ  effectuÃ©s
#     renvoie la liste des piÃ¨ces rendues"""
#
#
#     # cas de base : si la somme Ã  rendre est nulle, elle nÃ©cessite zÃ©ro piÃ¨ces
...
#     nb_pieces_a_rendre=s      #s=1+1+1+...1 dans le pire des cas
#     #pour chaque piÃ¨ce possible rendue
#     for p in pieces:
#         if s-p >=0 :
#             # le min permet de rÃ©initialiser le nb de piÃ¨ces Ã  rendre
#             nb_pieces_a_rendre,dico_solt[s] > len(dico_nb[s]
#             dico_solt[s] = rendre_monnaie_memo(pieces,s-p,dico_nb,dico_solt)+[s]
#        # si p>s : on ne fait rien (la combinaison examinÃ©e Ã©choue)
#     return dico_solt[s]

def nb_rendu_dyn(pieces,s):
    """prend en parametre le systeme de pieces (tuple) et s, la somme a  rendre,
    et utilise tab un tableau dans lequel on enregistre les tailles des solutions optimales calculees iterativement
    renvoie la *taille de* la liste optimale des pieces rendues"""
    tab=[0]*(s+1)
    ...
    for n in range(1,s+1):
        ...
        for p in pieces :
            if p<=n :
                ...
                # L'appel recursif de la version memo est remplace par un acces au tableau tab
    return tab[s]

def rendre_monnaie_dyn(pieces,s):
    """prend en parametre le systeme de pieces (tuple) et s, la somme a  rendre,
    et utilise tab_solt un tableau dans lequel on enregistre les solutions optimales calculees iterativement
...
if __name__ == '__main__' :
    ...
    # vÃ©rifie que le rendu glouton est le meilleur avec les systeme de piÃ¨ces de l'euro
    ...
    print("rendu glouton 8 avec (1,4,5) : ",glouton.rendre_monnaie((1,4,5),8))
    print("nb de pieces optimal pour rendre 8 :",nb_rendu_memo((1,4,5),8) )
    #compteur d'appels recursifs
    print_appels=True
    print("rendu glouton 4 avec (1,2,5) : ",glouton.rendre_monnaie((1,2,5),4))
